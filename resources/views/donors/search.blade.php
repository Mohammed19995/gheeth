                    ...
                    <div class="col-md-12">
                        ...
                        <div class="portlet light bordered">
                            <div class="portlet-title">
                                <form role="form" method="post" action="{{url('searchDonor')}}">
                                    {{csrf_field()}}
                                    <div class="form-body">
                                        <div class="row" style="margin-top: 20px;">
                                            ...
                                            <div class="col-sm-3">
                                                <div class="form-group  changeMarPad">
                                                    <label for="singleM1" class="control-label">اسم المتبرع</label>
                                                    <select id="single"
                                                            class="form-control select2 select2-hidden-accessible selectDonor"
                                                            tabindex="-1" aria-hidden="true" name="selectDonor">
                                                        <option></option>
                                                        @foreach($donors as $donor)
                                                            <option value="{{$donor->id}}" {{$donor->id==$selectDonor ? 'selected' : ''}}>{{$donor->name}}</option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <div class="col-sm-3">
                                                <div class="form-group  changeMarPad">
                                                    ...
                                                    <select id="single2"
                                                            class="form-control select2 select2-hidden-accessible selectِAliasDonor"
                                                            tabindex="-1" aria-hidden="true" name="selectAliasDonor">
                                                        <option></option>
                                                        @foreach($donors as $donor)
                                                            <option value="{{$donor->id}}" {{$donor->id==$selectDonor ? 'selected' : ''}}>{{$donor->alias_name}}</option>
                                                        @endforeach
                                                    </select>
                                                    <span class="help-block"></span>
                                                </div>
                                            ...
                                            <div class="col-sm-2">
                                                <label for="form_control_1">رصيد المتبرع</label>
                                                <div
                                                        class="input-group input-daterange ">
                                                    <input type="text" name="accountFrom" autocomplete="off"
                                                           value="{{empty($result) ? '': $accountFrom }}"
                                                           class="form-control">
                                                    <span class="input-group-addon form-group form-md-line-input has-success"> الى </span>
                                                    <input type="text" name="accountTo" autocomplete="off"
                                                           value="{{empty($result) ? '': $accountTo }}"
                                                           class="form-control">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $(".selectDonor").change(function() {
                if($(this).val() == '') {
                    $(".selectِAliasDonor").prop('disabled' , false);
                }else {
                    $(".selectِAliasDonor").prop('disabled' , true);
                }
            });

            $(".selectِAliasDonor").change(function() {
                if($(this).val() == '') {
